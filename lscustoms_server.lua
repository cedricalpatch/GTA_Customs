...
TriggerEvent('GTA_Inventaire:GetItemQty', source, "cash", function(qtyItem, itemid)
	if button.price then -- check if button have price
		local cash = qtyItem
		if(tonumber(cash) >= button.price) then
			TriggerClientEvent("GTA_Inventaire:RetirerItem", source, "cash", button.price)
			TriggerClientEvent('nMenuNotif:showNotification', source, " Vous payez : ~r~"..button.price)
